...
public class IntWeightedDirectedMultiGraph extends java.util.AbstractCollection<IIntWeightedDirectedMultiGraph.Edge>
implements IIntWeightedDirectedMultiGraph<IIntWeightedDirectedMultiGraph.Edge> {
	...
	private class IntEdge implements Edge {
		final int source, target;
		final int weight;

		IntEdge(int source, int target, int weight) {
			this.source = source;
			this.target = target;
			this.weight = weight;
		}
		...
		@Override
		public int getWeightAsInt() { return weight; }
		...
		@Override
		public int hashCode() {
			return source ^ target ^ Integer.hashCode(weight);
		}
	...
	private class AdjacencyArray extends Adjacency {
		...
		public Adjacency reverse() {
			return new AdjacencyArray(size(),
					getEdges().stream().map(e -> new IntEdge(e.getTarget(), e.getSource(), e.getWeight()))
	...
	private IntWeightedDirectedMultiGraph(int order, Adjacency adjacency) {
		this.order = order;
		this.adjacency = adjacency;
	}

	private IntWeightedDirectedMultiGraph(int order, java.util.Collection<? extends Edge> edge, Adjacent adjacent) {
		this.order = order;
		switch(adjacent) {
			case ARRAY:
	...
	public boolean addEdge(int source, int target, int weight) {
		return adjacency.addEdge(new IntEdge(source, target, weight));
	}
	...
	@Override
	public IntWeightedDirectedMultiGraph reverse() {
		return new IntWeightedDirectedMultiGraph(order, adjacency.reverse());
	}
	...
	public static IntWeightedDirectedMultiGraph create(int order) {
		return create(order, java.util.Collections.emptyList(), Adjacent.LIST);
	}

	public static IntWeightedDirectedMultiGraph create(int order, Adjacent adjacent) {
		return create(order, java.util.Collections.emptyList(), adjacent);
	}

	public static IntWeightedDirectedMultiGraph create(int order, java.util.Collection<? extends Edge> edge) {
		return create(order, edge, Adjacent.LIST);
	}

	public static IntWeightedDirectedMultiGraph create(int order, java.util.Collection<? extends Edge> edge, Adjacent adjacent) {
		...
		return new IntWeightedDirectedMultiGraph(order, edge, adjacent);
	}
}
