...
public class DoubleWeightedDirectedMultiGraph extends java.util.AbstractCollection<IDoubleWeightedDirectedMultiGraph.Edge>
implements IDoubleWeightedDirectedMultiGraph<IDoubleWeightedDirectedMultiGraph.Edge> {
	...
	private class DoubleEdge implements Edge {
		final int source, target;
		final double weight;

		DoubleEdge(int source, int target, double weight) {
			this.source = source;
			this.target = target;
			this.weight = weight;
		}
		...
		@Override
		public double getWeightAsDouble() { return weight; }
		...
		@Override
		public int hashCode() {
			return source ^ target ^ Double.hashCode(weight);
		}
	...
	private class AdjacencyArray extends Adjacency {
		...
		public Adjacency reverse() {
			return new AdjacencyArray(size(),
					getEdges().stream().map(e -> new DoubleEdge(e.getTarget(), e.getSource(), e.getWeight()))
	...
	private DoubleWeightedDirectedMultiGraph(int order, Adjacency adjacency) {
		this.order = order;
		this.adjacency = adjacency;
	}

	private DoubleWeightedDirectedMultiGraph(int order, java.util.Collection<? extends Edge> edge, Adjacent adjacent) {
		this.order = order;
		switch(adjacent) {
			case ARRAY:
	...
	public boolean addEdge(int source, int target, double weight) {
		return adjacency.addEdge(new DoubleEdge(source, target, weight));
	}
	...
	@Override
	public DoubleWeightedDirectedMultiGraph reverse() {
		return new DoubleWeightedDirectedMultiGraph(order, adjacency.reverse());
	}
	...
	public static DoubleWeightedDirectedMultiGraph create(int order) {
		return create(order, java.util.Collections.emptyList(), Adjacent.LIST);
	}

	public static DoubleWeightedDirectedMultiGraph create(int order, Adjacent adjacent) {
		return create(order, java.util.Collections.emptyList(), adjacent);
	}

	public static DoubleWeightedDirectedMultiGraph create(int order, java.util.Collection<? extends Edge> edge) {
		return create(order, edge, Adjacent.LIST);
	}

	public static DoubleWeightedDirectedMultiGraph create(int order, java.util.Collection<? extends Edge> edge, Adjacent adjacent) {
		...
		return new DoubleWeightedDirectedMultiGraph(order, edge, adjacent);
	}
}
