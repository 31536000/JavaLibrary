...
public class AVLTree<E> extends AbstractBinarySearchTree<E> {
	Node first, last;
	NilNode nil;
	Node.Edge tree;
	...
	public class DatNode extends Node {
		...
		public void leftRotate() {
			right.next.parent = parent;
			Node tmp = parent.next = right.next;
			right.next = right.prev;
			right.prev = tmp;
			parent = right;
			right = parent.prev.left;
			parent.prev.left = parent;
			right.prev = this;
			calc();
			parent.prev.calc();
		}
		...
		public void rightRotate() {
			left.next.parent = parent;
			Node tmp = parent.next = left.next;
			left.next = left.prev;
			left.prev = tmp;
			parent = left;
			left = parent.prev.right;
			parent.prev.right = parent;
			left.prev = this;
			calc();
			parent.prev.calc();
		}
		...
		public void rotate() {
			Node left = this.left.next, right = this.right.next;
			if (left.height >= right.height + 2) { // 右回転して高さを調節する
				if (left.left.next.height < left.right.next.height) left.leftRotate(); // 一度、左回転を行う二重回転型
				rightRotate();
			} else if (left.height + 2 <= right.height) { // 左回転して高さを調節する
				if (right.left.next.height > right.right.next.height) right.rightRotate(); // 一度、右回転を行う二重回転型
				leftRotate();
			}
			calc();
		}
		...
		public void calcSize() {
			size = left.next.size + right.next.size + 1;
		}
