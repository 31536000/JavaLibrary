 ...
 * これは、Consumerに対してプリミティブ型特殊化(long向け)を行ったものです。
 * LongConsumerは他のほとんどの関数型インタフェースと異なり、副作用を介して動作することを期待されます。
 * <p>
 * これは、{@link #accept(long)}を関数メソッドに持つ関数型インタフェースです。
 * @author 31536000
 *
 */
public interface LongConsumer extends Consumer<Long>, java.util.function.LongConsumer{
	...
	@Override
	public default void accept(Long t) {
		accept((long)t);
	}
	/**
	 * このオペレーションを実行した後、続けてafterオペレーションを実行する合成LongConsumerを返します。
	 ...
	 * @return このオペレーションを実行した後、続けてafterオペレーションを実行する合成LongConsumer
	 * @exception NullPolongerException afterがnullの場合
	 */
	public default LongConsumer andThen(LongConsumer after) {
		LongConsumer last = this;
