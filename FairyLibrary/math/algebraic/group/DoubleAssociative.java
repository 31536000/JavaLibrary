 ...
 * @author 31536000
 */
public interface DoubleAssociative extends Associative<Double>, DoubleMagma {
	@Override
	...
	public default double hyperAsDouble(double element, int repeat) {
		...
		double ret = element;
		--repeat;
		for (double mul = element; repeat > 0; repeat >>= 1, mul = applyAsDouble(mul, mul))
			if ((repeat & 1) != 0) ret = applyAsDouble(ret, mul);
		return ret;
	}
